...
function timeDiff(secondstime, date) {
  ...
  let dateTimeHours = new Date(date).addSeconds(secondstime);
  // DATE FORMAT
...
function timer(id, size, duration, startdate, mail, on_process) {
  ...
  if (duration) {
    secondstime =
      parseInt(duration.split(":")[0]) * 60 * 60 +
  ...
  const timerUpdate = setInterval(() => {
    let times = timeDiff(secondstime, startdate);
    let hours = times[0];
    let minutes = times[1];
    let seconds = times[2];
